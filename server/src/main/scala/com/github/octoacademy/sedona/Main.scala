...
import com.github.octoacademy.sedona.configuration.Application
import com.github.octoacademy.sedona.module.db.{LiveLodgingRepository, LodgingRepository}
import com.github.octoacademy.sedona.module.logger.{ConsoleLogger, Logger => MyLogger}
import com.github.octoacademy.sedona.route.LodgingRoute
...
object Main extends App {
  type AppEnvironment = Clock with Console with LodgingRepository with MyLogger
  private val lodgingRoute = new LodgingRoute[AppEnvironment]
  private val yaml = lodgingRoute.getEndPoints.toOpenAPI("User", "1.0").toYaml
  ...
  override def run(args: List[String]): ZIO[Main.Environment, Nothing, Int] = {
    val result = for {
      ...
      httpApp = Router(
        "/" -> lodgingRoute.getRoutes,
        "/docs" -> new SwaggerHttp4s(yaml).routes[TaskR[AppEnvironment, ?]]
      ).orNotFound
      finalHttpApp = Logger.httpApp[ZIO[AppEnvironment, Throwable, ?]](logHeaders = true, logBody = true)(httpApp)
      ...
      program <- server.provideSome[Environment] { base =>
        new Clock with Console with LiveLodgingRepository with ConsoleLogger {
